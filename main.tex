...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% select the basic options of this thesis/dissertation (this document will use thesis for convenience)
%% 选择论文的基本类型

\documentclass[
    master,     % 必选项：   {master, doctor} 此处不区分专业/学术学位，在下面学科类型处区分 
                % Mandatory: {master, doctor} No difference between Academic degree and Professional degree, 
                %                             but be careful about the option `\degree' and `subject'
    openany,    % 必选项：   {openany, openright} (增加空白页以适应双面打印)
                % Mandatory: {openany, openright} openright for printing, it will generate blank page to accomodate two-side printing
    % english,  % 可选项：   英文正文请选择此项
                % Optional:  For english main content, It will not generate Titlepage and Table of Contents in Chinese, 
                %            and change some auto-generated matter into English
    % blind,    % 可选项：   论文用于盲审请选择此项，将不会产生致谢(全文)、答辩委员会会议决议(内容)、
                %            常规评阅人名单(内容)，注意请自行修改 `攻读学位期间取得的研究成果' 内的内容格式
                % Optional:  For blind review, It will not generate Acknowledgements(All), Decision of Defense Committee(Content), 
...
\title{一二三四五六七八九十一二三四五六七八九十一二三四五六七八九十一二三四五}{English Title and English Title and English Title}
...
\advisorassociate{姓名2}{教授}{Name 2}{Prof.}
...
\addcommitteemember{西安交通大学,四五六七,教授}
\addcommitteemember{西安理工大学,一二三,教授}
\addcommitteemember{国家电网西北分部,二三四,高工}
\addcommitteemember{西安交通大学,三四五,副教授}
\addcommitteemember{西安交通大学,五六,教授}
...
\addgeneralreviewer{西安交通大学,四五六七,教授}
\addgeneralreviewer{西安理工大学,一二三,教授}
\addgeneralreviewer{国家电网西北分部,二三四,高工}
\addgeneralreviewer{西安交通大学,三四五,副教授}
\addgeneralreviewer{西安交通大学,五六,教授}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 注意：根据校方要求，以下所有页面顺序不可调整，英文写作的论文要求请咨询后自行调整
%% Notice: The order of these pages are defined in the requirements of the University, If the thesis written in english has a different requriment, please adjust it by yourself, if these configuration cannot fullfill the requirements, tell me in github issues.
...
% [自动生成] 生成摘要页 
% [Auto Generate] Abstract [If you select `english' option, it will only generate english abstract]
\thesisabstract

% [自动生成] 中英目录页（开启英文选项后只有英文目录）
% [Auto Generate] Table of Contents(in Chinese and English, If you enable the `english` option in the document class, there will be only English ToC)
\thesistableofcontens

% [自动生成] 主要符号表 修改 Main_Miscellaneous/glossary.tex 中的内容
% [Auto Generate] Glossaries Page, Rewrite you content in Main_Miscellaneous/glossary.tex 
\thesisglossarylist

% 正文 注意：英文正文写作时，中、英标题都填写为英文标题，同时，参数的顺序有意义，不要乱放
% Main contents, X of cX is the chapter of the thesis, you can change it if you want, the order matters
% and FILL THE BOTH CHINESE AND ENGLISH NAME OF TITLES IN ENGLISH IF YOU USE `english' option
\thesisbody{
    Main_Spine/c1,
    Main_Spine/c2,
    Main_Spine/c3,
...
% Acknowledgement, Rewrite you content in Main_Miscellaneous/acknowledegment.tex
...
% Achievement, Rewrite you content in Main_Miscellaneous/achievement.tex
\thesisachivements

% 答辩委员会决议 修改 Main_Miscellaneous/decision.tex 中的内容 注意盲审时需要手动修改格式
% Decision, Rewrite you content in Main_Miscellaneous/decision.tex
\thesisdecision

% [自动生成] 常规评阅人名单
% [Auto Generate] Reviewer List
\thesisreviewers
